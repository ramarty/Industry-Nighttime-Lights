...
# Load Data --------------------------------------------------------------------
# N_firms_sum_all_sum_splagunit_log
can_sub <- can %>%
  dplyr::filter(unit %in% "Grid in Cities")

can$dmspolsharmon_median_splagunit %>% summary()
can$N_firms_sum_all_sum_splagunit_log %>% summary()

lm(dmspolsharmon_mean_log ~ N_firms_sum_all_log + N_firms_sum_all_sum_splagunit_log + factor(year), data = can) %>%
  summary()

cor.test(can$N_firms_sum_all_log,
    can$N_firms_sum_all_mean_splagunit_log)
...
can %>% 
  #filter(unit %in% "5km Grid") %>%
  ggplot() +
  geom_point(aes(x = N_firms_sum_all_log_diff6,
                 y = dmspolsharmon_mean_log_diff6),
             alpha = 0.1) +
  facet_wrap(~unit)
