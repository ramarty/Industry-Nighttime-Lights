  ...
  for(ntl_var in c("dmspolsharmon_mean_log", "viirs_mean_log")){
    for(unit in c("5km Grid", "10km Grid", "25km Grid", "50km Grid", "100km Grid", "City" ,"Grid in Cities")){
      for(country in c("mexico", "canada")){
        ...
        dvindust_lm_yr   <- felm(industry_var ~ ntl_var | year      | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "yr", splag = F) 
        dvindust_lm_id   <- felm(industry_var ~ ntl_var | id        | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "id", splag = F) 
        dvindust_lm_yrid <- felm(industry_var ~ ntl_var | year + id | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "year_id", splag = F) 
        
        dvindust_lag_lm_yr   <- felm(industry_var ~ ntl_var + ntl_var_splag | year      | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "yr", splag = T) 
        dvindust_lag_lm_id   <- felm(industry_var ~ ntl_var + ntl_var_splag | id        | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "id", splag = T) 
        dvindust_lag_lm_yrid <- felm(industry_var ~ ntl_var + ntl_var_splag | year + id | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "year_id", splag = T) 
        ...
        ## Regressions - NTL DV
        dvntl_lm_yr   <- felm(ntl_var ~ industry_var | year      | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "yr", splag = F) 
        dvntl_lm_id   <- felm(ntl_var ~ industry_var | id        | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "id", splag = F) 
        dvntl_lm_yrid <- felm(ntl_var ~ industry_var | year + id | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "year_id", splag = F) 
        
        dvntl_lag_lm_yr   <- felm(ntl_var ~ industry_var + industry_var_splag | year      | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "yr", splag = T) 
        dvntl_lag_lm_id   <- felm(ntl_var ~ industry_var + industry_var_splag | id        | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "id", splag = T) 
        dvntl_lag_lm_yrid <- felm(ntl_var ~ industry_var + industry_var_splag | year + id | 0 | 0, data = df[df$unit %in% unit,]) %>% extract_coefs() %>% mutate(FE = "year_id", splag = T) 
