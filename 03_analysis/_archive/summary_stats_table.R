...
make_stats <- function(df, var, year){
  ...
  if(var %in% "dmspolsharmon_mean") varname <- "DMSP-OLS"
  if(var %in% "viirs_mean")         varname <- "VIIRS"
  if(var %in% "dmspols_sum")        varname <- "DMSP-OLS"
  if(var %in% "dmspolsharmon_sum")  varname <- "DMSP-OLS"
  if(var %in% "viirs_sum")          varname <- "VIIRS"
