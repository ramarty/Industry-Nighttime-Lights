...
# Load Data --------------------------------------------------------------------
mex_viirs_df <- readRDS(file.path(data_file_path, "Grid", "FinalData", "mexico", "merged_datasets", paste0("hex_",GRID_SIZE,"km_viirs_clean.Rds")))
mex_dmspols_df <- readRDS(file.path(data_file_path, "Grid", "FinalData", "mexico", "merged_datasets", paste0("hex_",GRID_SIZE,"km_dmspols_clean.Rds")))
can_df <- readRDS(file.path(data_file_path, "Grid", "FinalData", "canada", "merged_datasets", paste0("hex_",GRID_SIZE,"km_clean.Rds")))

# Log variables not logged -----------------------------------------------------
vars <- names(mex_viirs_df)[!grepl("log", names(mex_viirs_df))]
for(v in vars) if(is.numeric(mex_viirs_df[[v]])) mex_viirs_df[[paste0(v, "_log")]] <- log(mex_viirs_df[[v]]+1)

vars <- names(mex_dmspols_df)[!grepl("log", names(mex_dmspols_df))]
for(v in vars) if(is.numeric(mex_dmspols_df[[v]]))  mex_dmspols_df[[paste0(v, "_log")]] <- log(mex_dmspols_df[[v]]+1)

vars <- names(can_df)[!grepl("log", names(can_df))]
for(v in vars) if(is.numeric(can_df[[v]]))  can_df[[paste0(v, "_log")]] <- log(can_df[[v]]+1)

if(F){
  df_dmspols <- can_df
  df_viirs <- can_df
  dmspols_var <- "dmspolselvidge_mean_log"
  viirs_var <- "viirs_mean_log"
  indep_var <- "N_firms_sum_"
}
