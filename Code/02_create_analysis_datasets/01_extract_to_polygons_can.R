...
for(dataset in c(grid_files,
  ...
  if(grepl("hex", dataset)){
    ...
    OUT_PATH <- file.path(data_file_path, "Grid", "FinalData", "canada", "individual_datasets")
  }
  ...
  if(grepl("gadm", dataset)){
    ...
    OUT_PATH <- file.path(data_file_path, "GADM", "FinalData", "canada", "individual_datasets")
  }
  ...
  #### DMSPOLS
  if(EXTRACT_DMSPOLS){
    polygon_dmspols <- lapply(FIRM_YEARS, extract_dmspols, polygon, "canada") %>% bind_rows()
