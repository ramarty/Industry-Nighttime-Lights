...
#### DMSP-OLS
dmspols <- raster(file.path(data_file_path, "Nighttime Lights", "DMSPOLS", "mexico_dmspols_2013.tif"))
dmspols_df <- dmspols %>%
  as("SpatialPixelsDataFrame") %>%
  as.data.frame() %>%
  dplyr::rename(value = mexico_dmspols_2013,
                lon = x,
                lat = y) 

dmspols_df <- dmspols_df %>%
  dplyr::filter(value > 0) %>%
  dplyr::select(lon, lat)

#### Coordinates
coords_df <- bind_rows(firms_coords, 
                       dmspols_df) %>%
  mutate(id = 1:n())

coordinates(coords_df) <- ~lon+lat
crs(coords_df) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
coords_df <- spTransform(coords_df, CRS(PROJ_mexico))
...
crs(coords_df_coords_dnct) <- CRS(PROJ_mexico)
...
# Make Hexagons ----------------------------------------------------------------
gadm <- readRDS(file.path(data_file_path, "GADM", "RawData", "gadm36_MEX_0_sp.rds"))
gadm <- gSimplify(gadm, tol = .1)
gadm <- spTransform(gadm, CRS(PROJ_mexico))
gadm$id <- 1
...
saveRDS(hex_10,   file.path(data_file_path, "Grid", "RawData","mex_hex_10km.Rds"))

hex_5    <- create_hexagons(hex_10,   5*1000,    T, coords_buff_agg_sf)
saveRDS(hex_5,    file.path(data_file_path, "Grid", "RawData","mex_hex_5km.Rds"))

#hex_1    <- create_hexagons(hex_5,    1*1000,    F, coords_buff_agg_sf)
#saveRDS(hex_1,    file.path(data_file_path, "Grid", "RawData","mex_hex_1km.Rds"))

# Export -----------------------------------------------------------------------
saveRDS(hex_1000, file.path(data_file_path, "Grid", "RawData","mex_hex_1000km.Rds"))
saveRDS(hex_500,  file.path(data_file_path, "Grid", "RawData","mex_hex_500km.Rds"))
saveRDS(hex_250,  file.path(data_file_path, "Grid", "RawData","mex_hex_250km.Rds"))
saveRDS(hex_100,  file.path(data_file_path, "Grid", "RawData","mex_hex_100km.Rds"))
saveRDS(hex_50,   file.path(data_file_path, "Grid", "RawData","mex_hex_50km.Rds"))
saveRDS(hex_25,   file.path(data_file_path, "Grid", "RawData","mex_hex_25km.Rds"))
saveRDS(hex_10,   file.path(data_file_path, "Grid", "RawData","mex_hex_10km.Rds"))
saveRDS(hex_5,    file.path(data_file_path, "Grid", "RawData","mex_hex_5km.Rds"))
#saveRDS(hex_1,    file.path(data_file_path, "Grid", "RawData","mex_hex_1km.Rds"))
